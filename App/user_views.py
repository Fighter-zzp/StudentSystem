...
from utils.check_utils import is_login
...
def login():
    ...
    if request.method == 'POST':
        ...
        if not all([username, password]):
            return render_template('login.html', msg='请填写用户名或密码')
        # 查询用户
...
def grade_students_list():
    ...
    if request.method == 'GET':
        ...
        paginate = Student.query.filter(Student.grade_id == g_id).paginate(page, page_num)
        stus = paginate.items
